...
function findGalaxies(universe) {
  let galaxyCoords = {}, galaxyCount = 0
...
function run() {
  ...
  const emptyCols = Array.from(Array(universe[0].length).keys()).filter(y => !galaxyArray.find(el => el[0] === y))
  const emptyRows = Array.from(Array(universe.length).keys()).filter(x => !galaxyArray.find(el => el[1] === x))
  // expand empty
  if (!PART_2) {
    let addedRows = 0
    for (let emptyIdx of emptyRows) {
      universe.splice(emptyIdx + addedRows++, 0, Array(universe[0].length).fill("."))
    }
    let addedCols = 0
  ...
  for (let galaxyCoord of galaxies) {
    ...
    galaxies.filter(coord => !complete.find(crd => crd === coord)).forEach(coord => {
      const x1 = galaxyCoord[0], y1 = galaxyCoord[1]
      const x2 = coord[0], y2 = coord[1]
      const lowX = Math.min(x1, x2), lowY = Math.min(y1, y2)
      const highX = Math.max(x1, x2), highY = Math.max(y1, y2)
